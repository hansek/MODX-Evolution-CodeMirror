...
if (isset ($_POST['tv'])) {
    ...
    foreach ($selTVs as $si) {
        ...
        if (count($assignments) > 0) {
            $ds = mysql_query("SELECT id FROM $dbase.`".$table_prefix."site_tmplvars` WHERE name='$name' AND description='$desc' LIMIT 1;",$sqlParser->conn);
            if($ds){
              $tvid = mysql_result($ds,0);
              // remove existing tv -> template assignements
              mysql_query('DELETE FROM ' . $dbase . '.`' . $table_prefix . 'site_tmplvar_templates` WHERE tmplvarid = \'' . $tvid . '\'');
              ...
              foreach ($assignments as $assignment) {
                  ...
                  if ($ts) {
                      ...
                      mysql_query("INSERT INTO $dbase.`" . $table_prefix . "site_tmplvar_templates` (tmplvarid, templateid) VALUES($tvid, $templateId)");
                 }
              }
            }
        }
